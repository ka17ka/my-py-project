...
def test_special_hardware():
    case_data = common.get_test_data()["check_special_hardware_byname_cases"]
    response = common.req_api(case_data)
    res = response.json()
    if not res.get("data",{}).get("records"):
        case_data = common.get_test_data()["check_special_hardware_bycode_cases"]
        response = common.req_api(case_data)
        res = response.json()

    special_hardware_id = None
    expect_id = case_data["expect"]["data"]["id"]
...
def test_special_lamppost():
    case_data = common.get_test_data()["check_special_lamppost_cases"]
    response = common.req_api(case_data)
    res = response.json()
