...
def write_cached_token(token,expires_at):
    ...
    :return:
    """
    with open('config/cache_token.json', 'w') as f:
...
def read_cached_token():
    ...
    """
    if not os.path.exists("config/cache_token.json"):
        return None,0
    try:
        with open('config/cache_token.json', 'r') as f:
            data = json.load(f)
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if res["data"]["records"]:
                ...
                if pos1_switchstatus or pos2_switchstatus:
                        return True
...
def check_lampbase_open_switchstatus(test_special_lamppost,start_time,current_end_time,timeout=15,interval=2):
    """
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            response = common.req_api(case_data, param=param)
            res = response.json()
            # print(f"当前时间段上报数据:{res.get('data')}")
            if res["data"]["records"]:
                pos1_switchstatus = None
                pos2_switchstatus = None
                for r in res["data"]["records"]:
                    if r["pstion"] == 1:
                        ...
                        pos2_switchstatus = r["switchStatus"]
                if pos1_switchstatus == expect_open_switchstatus or pos2_switchstatus == expect_open_switchstatus:
...
def check_lampbase_close_switchstatus(test_special_lamppost, start_time, current_end_time, timeout=15, interval=2):
    """
            ...
            if res["data"]["records"]:
                ...
                for r in res["data"]["records"]:
                    if r["pstion"] == 1:
                        ...
                        pos2_switchstatus = r["switchStatus"]
                if pos1_switchstatus == expect_close_switchstatus or pos2_switchstatus == expect_close_switchstatus:
                # if pos1_switchstatus == expect_close_switchstatus:
                    return True
                # elif pos1_switchstatus == expect_close_switchstatus and pos2_switchstatus == expect_close_switchstatus:
                # if pos1_switchstatus == expect_close_switchstatus:
...
def check_lampbase_dimming(test_special_lamppost,start_time,current_end_time,timeout=15,interval=2):
    """
    ...
    while time.time() - start_time < timeout:
        try:
            ...
            if res["data"]["records"]:
                ...
                if pos1_dimminglevel == expect_main_dimminglevel or pos2_dimminglevel == expect_side_dimminglevel:
